...
export function SidebarLayout({ children, isOwner }: SidebarLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      >
        <div className="px-6 py-7 border-b border-[var(--sidebar-border)]">
          <Link
            to="/"
            onClick={() => setMobileOpen(false)}
            className="flex items-center gap-3 group"
          >
            <div className="h-12 w-12 rounded-2xl bg-[var(--primary)] flex items-center justify-center shrink-0 transition-transform group-hover:scale-105 duration-200">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 339 328" className="w-8 h-8">
                <g transform="translate(0,328) scale(0.1,-0.1)" fill="var(--primary-foreground)" stroke="none" strokeWidth="25" strokeLinejoin="round" strokeLinecap="round">
            ...
            <div className="min-w-0">
              <h1 className="font-serif text-[15px] font-semibold text-[var(--sidebar-foreground)] leading-tight truncate">
                Native Camping
              </h1>
              <p className="text-[10px] uppercase tracking-wider text-[var(--muted-foreground)] leading-tight truncate">
                & Restaurant
              </p>
            </div>
          </Link>
        ...
        <div className="p-4 border-t border-[var(--sidebar-border)] space-y-2">
          <div className="flex items-center gap-2.5 px-2 py-2 text-xs text-[var(--muted-foreground)] group">
            <div className="h-8 w-8 rounded-full bg-[var(--primary)] flex items-center justify-center shrink-0 transition-transform group-hover:scale-105 duration-200">
              <span className="text-[11px] font-semibold text-[var(--primary-foreground)]">
                {user?.name?.charAt(0)?.toUpperCase() || "U"}
              </span>
            </div>
            <span className="truncate font-medium text-[var(--sidebar-foreground)]">{user?.name || "User"}</span>
            <span className="ml-auto text-[9px] font-semibold uppercase tracking-wider bg-[var(--primary)]/12 text-[var(--primary)] px-2 py-1 rounded-full">
      ...
      <div className="flex-1 flex flex-col lg:ml-72 min-w-0">
        <header className="sticky top-0 z-30 bg-[var(--background)]/75 backdrop-blur-xl border-b border-[var(--border)] px-5 lg:px-8 py-3.5 flex items-center gap-4">
          <button
        ...
        <main className="flex-1 p-5 lg:p-8 overflow-x-hidden max-w-[1600px] w-full mx-auto">
          {navigatingTo ? (
